...
public class MaterielController {
    ...
    public ResponseEntity<Materiels> saveMateriel(
        ...
        @Valid @RequestParam(value = "image", required = false)  MultipartFile imageFile) throws Exception
    {
       ...
       try {
        materiel =  new JsonMapper().readValue(addMateriel, Materiels.class);
       } catch (JsonProcessingException e) {
           throw new Exception(e.getMessage());
       }
       Materiels savedMateriel = materielService.createMateriel(materiel, imageFile);
...
private MediaType detectContentType(String imageName) {
    ...
    public ResponseEntity<Page<Materiels>> getMaterielByTypeMaterielWithPagination(
        ...
        @RequestParam() int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Materiels> materiels = materielService.getMaterielByTypeMaterielWithPagination(idTypeMateriel,pageable);
        return ResponseEntity.ok().body(materiels);
    }

    @GetMapping("/getAllMaterielsByActeurWithPagination")
    public ResponseEntity<Page<Materiels>> getMaterielByActeurWithPagination(
        @RequestParam() String idActeur,
        @RequestParam() int page,
        @RequestParam() int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Materiels> materiels = materielService.getMaterielByActeurWithPagination(idActeur,pageable);
        return ResponseEntity.ok().body(materiels);
    }
        ...
        @Valid @RequestParam(value = "image", required = false)  MultipartFile imageFile, @PathVariable String id) throws Exception
    {
       ...
       Materiels savedMateriel = materielService.updateMateriel(materiel, id, imageFile);

       return new ResponseEntity<>(savedMateriel, HttpStatus.OK);
    }

    @PutMapping("/activer/{id}")
    public ResponseEntity<Materiels> activeMateriel(@PathVariable String id) throws Exception{
        return new ResponseEntity<>(materielService.active(id), HttpStatus.OK);
    }

    @PutMapping("/desactiver/{id}")
    public ResponseEntity<Materiels> desactiveMateriel(@PathVariable String id) throws Exception{
